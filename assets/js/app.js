    ...
    if ($inputSearch.val().length > 0 && fuse) {
      ...
      if (results.length > 0) {
        for (var i = 0, len = results.length; i < len; i++) {
          htmlString += "\n          <article class=\"m-result\">            <a href=\"".concat(results[i].item.url, "\" class=\"m-result__link\">              <h3 class=\"m-result__title\">").concat(results[i].item.title, "</h3>              <span class=\"m-result__date\">").concat(Object(_helpers__WEBPACK_IMPORTED_MODULE_7__["formatDate"])(results[i].item.published_at), "</span>            </a>          </article>");
        }
